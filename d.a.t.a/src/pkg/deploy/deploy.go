...
func (c *Client) DeployNFT(ctx context.Context, req DeployNFTRequest) (*DeployNFTResponse, error) {
    ...
    // If this is an init-collection response, try to auto-submit if we have secret
    if initResp.Tx != "" && c.OwnerSecret != "" {
        // Send to server submit endpoint for broadcasting
        base := strings.TrimRight(c.BaseURL, "/")
        submitURL := base + "/api/tx/submit"
        payload := map[string]interface{}{"tx": initResp.Tx, "secret": c.OwnerSecret}
