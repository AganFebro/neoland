...
type Config struct {
    ...
    // Deploy holds configuration for your internal deploy API/server
    Deploy struct {
        BaseURL string `mapstructure:"base_url"`
        APIKey  string `mapstructure:"api_key"`
        Path    string `mapstructure:"path"`
        Payer   string `mapstructure:"payer"`
        Owner   string `mapstructure:"owner"`
        OwnerSecret string `mapstructure:"owner_secret"`
        SavePinImages bool   `mapstructure:"save_pin_images"`
...
func loadEnvConfig(confPath string) error {
    ...
    viper.AddConfigPath(confPath)
    // Also allow loading .env from current working directory
    viper.AddConfigPath(".")
    // And pick up real environment variables if they're exported
    viper.AutomaticEnv()
    ...
    envMappings := map[string]string{
        ...
        "DEPLOY_API_BASE_URL":    "deploy.base_url",
        "DEPLOY_API_KEY":         "deploy.api_key",
        "DEPLOY_API_PATH":        "deploy.path",
        "DEPLOY_PAYER":           "deploy.payer",
        "DEPLOY_OWNER":           "deploy.owner",
        "DEPLOY_OWNER_SECRET":    "deploy.owner_secret",
        "SAVE_PIN_IMAGES":        "deploy.save_pin_images",
        "PIN_IMAGE_SAVE_DIR":     "deploy.save_dir",
