...
func NewSocialClient(
    ...
    if telegramConfig != nil && telegramConfig.Token != "" {
        ...
        if err != nil {
            // Don't crash the entire agent if Telegram token is invalid
            logger.GetLogger().Warnf("Failed to initialize Telegram client, continuing without Telegram: %v", err)
        } else {
...
func (sc *SocialClientImpl) monitorDiscord(ctx context.Context) {
    ...
    for {
        select {
        case msg := <-channel:
            // Normalize attachments into generic maps to avoid cross-package types in core
            ...
            sc.socialMsgChannel <- core.SocialMessage{
                ...
                Metadata: map[string]interface{}{"channel_id": msg.ChannelID, "attachments": atts},
            }
        case <-ctx.Done():
            return
        }
