...
pub mod collection {
    ...
    // v2 mint: same as `mint` but allows setting royalty bps and a custom URI
    // at creation time so off-chain indexers immediately see correct metadata.
    pub fn mint_v2(
        ctx: Context<MintNft>,
        nonce: u64,
        lock_metadata: bool,
        ...
        // Create Metadata with royalty and creator info; default to collection's URI when `uri` empty
        let final_uri = if uri.len() > 0 { uri } else { collection.metadata_uri.clone() };
        let create_meta_ix = mpl_ix::CreateMetadataAccountV3 {
            metadata: metadata_pda,
            mint: mint_pda,
            mint_authority: auth_pda,
        ...
        .instruction(mpl_ix::CreateMetadataAccountV3InstructionArgs {
            data: DataV2 {
                ...
                creators: Some(vec![Creator { address: collection.owner, verified: false, share: 100 }]),
                collection: None,
                uses: None,
            },
            is_mutable: true,
